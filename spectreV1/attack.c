...
int main(int argc, const char** argv) {
    /********************************find secret in shared memory start **********************************/
    int shm_fd = shm_open("/shared_memory", O_RDWR, 0666);
    if (shm_fd == -1) {
        perror("shm_open");
        return 1;
    }
    size_t secret_size = 1000;  // the length of secret is unknown，set it to a longer length if possible.
    ...
    close(shm_fd);
    /********************************find secret in shared memory end **********************************/

    printf("Getting '%s' in memory, address %p\n", (char*)shared_memory, shared_memory);
    size_t malicious_x = (size_t)((char*)shared_memory - (char*)array1); /* default for malicious_x */
    int score[2], len = strlen((char*)shared_memory);
    uint8_t value[2];
...
#endif
    munmap(shared_memory, secret_size);  // 解除内存映射
